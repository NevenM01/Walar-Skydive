-- Relax auto_merge_athletes_name_core anchor requirement:
-- Allow results_count > 0 to act as an anchor when user_id/FAI is missing.
-- This enables auto-merge for cases where one row has real results but no FAI/user,
-- and the other row is junk (0 results, no FAI/user).
...
begin
  ...
  loop
    ...
    -- Losers: only junk rows (no user, no FAI, 0 results)
