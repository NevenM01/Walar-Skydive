-- Update winner selection for auto_merge_athletes_name_dob:
    ...
    from (
      ...
      from (
        select
          ...
          case when base.fai_norm is not null then 1 else 0 end as has_fai
        from (
          select
            a.id,
            a.user_id,
      ...
      order by ranked0.has_user desc, ranked0.has_fai desc, ranked0.results_count desc, ranked0.created_at asc
      limit 1
    ) b;
