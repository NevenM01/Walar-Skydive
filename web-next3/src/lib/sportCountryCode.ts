/** Maps ISO 3166-1 alpha-2 → sport display code (IOC/FAI convention). */
...
/**
 * Returns the sport/IOC display code (3-letter) for a given ISO alpha-2 code.
 * Falls back to the original code if no mapping exists.
 */
export function toSportCode(iso2: string): string {
  const upper = iso2.trim().toUpperCase()
  return ALPHA2_TO_SPORT[upper] ?? upper
}

/**
 * Returns the ISO alpha-2 code from a sport code, for use with flagcdn.com.
 * If the code is already alpha-2, returns it as-is.
 */
